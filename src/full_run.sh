...
sed -i 's/^\(mates\.num-data-influence-model-update\s*=\s*\).*/\15/' pyproject.toml
...
mkdir -p logs/$datetime
sed -i 's/^\(mates\.state\s*=\s*\).*/\1true/' pyproject.toml
sed -i 's/^\(mates\.num-data-influence-model-update\s*=\s*\).*/\13/' pyproject.toml
# Change folder parameters in utils.py to result_metric2
sed -i "s#\(folder=\"result_metric/\)[^\"]*\(\")\)#\1${datetime}\2#g" fedllm/utils.py
...
mkdir -p logs/$datetime
sed -i 's/^\(mates\.state\s*=\s*\).*/\1true/' pyproject.toml
sed -i 's/^\(mates\.num-data-influence-model-update\s*=\s*\).*/\110/' pyproject.toml
