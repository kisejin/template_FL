...
class DatasetAbstract:
    ...
    def get_split_dataset(self, dataset):
        print(
            f">> ===== After processing, Dataset has {len(dataset)} examples. ====="
        )
        if len(dataset) > 10000:
            ...
            print(
                f">> ===== After split, Dataset1 has {len(ds_part1)} examples and Dataset2 has {len(ds_part2)} examples. ====="
            )
            list_dataset = []
            list_global_set = []
            for subset in [ds_part1, ds_part2]:
                ...
                ds = DatasetDict(
                    {
                        "train": Dataset.from_pandas(train).remove_columns(
                            ["__index_level_0__"]
                        ),
                        "test": Dataset.from_pandas(test).remove_columns(
                            ["__index_level_0__"]
                        ),
                    }
                )
            ...
            global_set = Dataset.from_pandas(global_test).remove_columns(
                ["__index_level_0__"]
            )
...
class GeneralDataset(DatasetAbstract):
    ...
    def _processing_data(self):
        ...
        for dataset_name in self.dataset_name:
            df = pd.DataFrame(
                super().get_dataset(
                    dataset_name=dataset_name, local_data_dir=None
                )["train"]
            )
        ...
        global_test_set_hete.update({self.metadata["domain"]: global_test})
...
class MathDataset(DatasetAbstract):
    ...
    def get_split_dataset(self, dataset):
        ...
        print(
            f">> ===== After processing, Dataset  has {len(dataset_train)} examples. ====="
        )
        if len(dataset_train) > 10000:
            ...
            print(
                f">> ===== After split, Dataset1 has {len(ds_train_part1)} examples and Dataset2 has {len(ds_train_part2)} examples. ====="
            )
            list_dataset = []
            for i in range(2):
                ds = DatasetDict(
                    {
                        "train": Dataset.from_pandas(
                            eval(f"ds_train_part{i+1}")
                        ).remove_columns(["__index_level_0__"]),
                        "test": Dataset.from_pandas(
                            eval(f"ds_test_part{i+1}")
                        ).remove_columns(["__index_level_0__"]),
                    }
                )
                list_dataset.append(ds)
            ...
            ds = DatasetDict(
                {
                    "train": Dataset.from_pandas(dataset_train).remove_columns(
                        ["__index_level_0__"]
                    ),
                    "test": Dataset.from_pandas(dataset_test).remove_columns(
                        ["__index_level_0__"]
                    ),
                }
            )
    ...
    def _processing_data(self):
        ...
        for dataset_name in self.dataset_name:
            ...
            if dataset_name == "TIGER-Lab/MathInstruct":
                ...
                df_train, df_test = pd.DataFrame(ds_tmp["train"]), pd.DataFrame(
                    ds_tmp["test"]
                )
...
class CodeDataset(DatasetAbstract):
    ...
    def __init__(self):
        list_dataset = [
            ...
            "kisejin/code-gen-multi-language",
            "shanjay/ds1000-s",
        ]
        ...
        for i in range(len(ds_list)):
            ...
            for split_name, ds in ds_dict.items():
                if downsample_rate < 1.0:
                    ...
                    ds_dict[split_name] = ds.shuffle(seed=42).select(
                        range(new_length)
                    )
